...
export default async function ResumesPage() {
  ...
  const displayName = (user.user_metadata?.full_name as string | undefined) ?? "there";
...
function EmptyState() {
  return (
    <div className="rounded-2xl bg-[var(--surface-raised)] p-16 text-center shadow-[var(--sh-1)]">
      ...
      <h2 className="font-headline mt-5 text-2xl font-normal">Your first resume, waiting.</h2>
      <p className="mx-auto mt-2 max-w-sm text-sm text-[var(--on-surface-muted)]">
        Compose from your career profile — tailor it for each role later.
      </p>
...
function idToVariant(id: string): "modern" | "two-col" {
  // Deterministic alternation based on id for visual variety
